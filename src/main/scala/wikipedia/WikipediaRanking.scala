...
object WikipediaRanking {
  ...
  // define the spark context from the spark conf
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    langs.map(l => (l, occurrencesOfLang(l, rdd))).sortBy(pair => pair._2).reverse
  }
  ...
  def main(args: Array[String]) {
    ...
    // result: Processing Part 1: naive ranking took 77751 ms.
    ...
    // Processing Part 2: ranking using inverted index took 46589 ms.
    // val langsRanked2: List[(String, Int)] = timed("Part 2: ranking using inverted index", rankLangsUsingIndex(index))

    /* Languages ranked according to (3) */
    // Processing Part 3: ranking using reduceByKey took 29984 ms.
